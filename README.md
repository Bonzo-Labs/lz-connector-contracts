...
## Bonzo OFT Instructions

This section provides step-by-step instructions for deploying and using the Bonzo OFT (Omnichain Fungible Token) across different chains.

### 1. Deploy Contracts

Deploy the Bonzo OFT contract to each desired chain:

```bash
# Deploy to Arbitrum Testnet
npx hardhat run deploy/NewOFT.ts --network arbitrum-testnet

# Deploy to Avalanche Testnet
npx hardhat run deploy/NewOFT.ts --network avalanche-testnet
```

### 2. Wire Contracts Together

After deployment, connect the contracts across chains using LayerZero's wiring command:

```bash
npx hardhat lz:oapp:wire --oapp-config layerzero.config.ts
```

This command will configure the necessary peer connections and message libraries for cross-chain communication.

### 3. Verify Connections

Check the peer connections between chains:

```bash
npx hardhat lz:oapp:peers:get --oapp-config layerzero.config.ts
```

This will display the current peer configurations for each chain pair.

### 4. Mint Tokens

Mint OFT tokens on any deployed chain:

```bash
# Mint on Arbitrum Testnet
npx hardhat run scripts/mint-oft.ts --network arbitrum-testnet

# Mint on Avalanche Testnet
npx hardhat run scripts/mint-oft.ts --network avalanche-testnet
```
...
```bash
# Send from Arbitrum to Avalanche
npx hardhat run scripts/send-oft.ts --network arbitrum-testnet

# Send from Avalanche to Arbitrum
npx hardhat run scripts/send-oft.ts --network avalanche-testnet
```

> [!NOTE]
> Make sure you have sufficient native tokens (ETH, AVAX) in your wallet for gas fees on both the source and destination chains.

> [!TIP]
> You can monitor the cross-chain transfer status using the transaction hash returned by the send command.
