...
async function main() {
	...
	// Get MockERC20 address from deployment file
	let mockERC20Address: string;
	try {
		mockERC20Address = await getContractAddress(currentNetworkName, "MockERC20");
		console.log(`Found MockERC20 address for ${currentNetworkName}: ${mockERC20Address}`);
	} catch (error: any) {
		console.error("Error getting MockERC20 address:", error.message);
		process.exit(1);
	}
	...
	try {
		...
		const baseOFTAdapter = await BaseOFTAdapter.deploy(mockERC20Address, endpointAddress, delegate);
		...
		console.log("Token Address:", mockERC20Address);
		console.log("Delegate:", await baseOFTAdapter.owner());
